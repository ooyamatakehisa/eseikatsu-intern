...
export default class BuildingQueryAPIApi {
    ...
    */
     ...
     * @param {Number} opts.latitudeFrom 緯度(世界測地系)検索区間（DEG形式。単位:ミリ秒）
     * @param {Number} opts.latitudeTo 緯度(世界測地系)検索区間（DEG形式。単位:ミリ秒）
     * @param {Number} opts.longitudeFrom 経度(世界測地系)検索区間（DEG形式。単位:ミリ秒）
     * @param {Number} opts.longitudeTo 経度(世界測地系)検索区間（DEG形式。単位:ミリ秒）
     * @param {Array.<Number>} opts.customerKey カスタマーキー
     * @param {Array.<String>} opts.tatemonoGuid One建物GUID
     * @param {Array.<String>} opts.buildingFullKey 建物フルID
    ...
    searchBuildingWithHttpInfo(opts) {
      ...
      let queryParams = {
        ...
        'building_full_key': this.apiClient.buildCollectionParam(opts['buildingFullKey'], 'multi')
      };
      let headerParams = {
      };
      let formParams = {
